...
//
    ...
    if ($vbulletin->userinfo['userid'] == $user_id) {
        $profilefields = $vbulletin->db->query_read_slave("
            SELECT *
            FROM " . TABLE_PREFIX . "profilefield
            WHERE editable > 0 AND hidden = 0
        ...
        while ($profilefield = $vbulletin->db->fetch_array($profilefields))
        {
            ...
            $value = $userinfo[$profilefieldname];
            ...
            if (in_array($profilefield['type'], array('radio', 'cbox')))
            {
                ...
                if ($profilefield['type'] == 'cbox')
                {
                    $count = 1;
                    while ($count <= strlen(decbin($value)))
                    {
                        if ($value & pow(2, $count - 1))
                        {
                            $real_value[$count] = new xmlrpcval(1, 'i4');
                        }
                        $count++;
                    }
                    $value = $real_value;
                    // print_r($field_res['value']);exit;
                }
            }
            ...
            $field_res = array(
                ...
                'require' => new xmlrpcval($profilefield['required'], 'boolean'),
            );
            ...
            if ($profilefield['type'] != 'cbox')
            {
                $field_res['value'] = new xmlrpcval(mobiquo_encode($value), 'base64');
            }
            else
            {
                $field_res['value'] = new xmlrpcval($value, 'struct');
            }
            $update_return_array[] = new xmlrpcval($field_res, 'struct');
        }
    }
    ...
    $return_user = array(
        ...
        'update_fields_list'    => new xmlrpcval($update_return_array, 'array'),
    ...
    fetch_avatar_from_userinfo($userinfo, false, false);
