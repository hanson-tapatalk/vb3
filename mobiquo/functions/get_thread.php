...
function get_thread_by_unread_func($xmlrpc_params)
{
    ...
    //not need take care of coventry to get the last unread, it only give us the post position and ignored post is valid too
        ...
        if ( $fetchtype == 'post' or $fetchtype == 'post_ignore' OR ($fetchtype == 'post_deleted' AND $show['viewpost']))
        {
            ...
            if(is_array($mobiquo_attachments)){
                foreach($mobiquo_attachments as $attach) {
                    ...
                    // https://www.campingforen.de/foren/test/30467-test-fuer-tapatalk.html
                    // fix the attachment url issue, the board url has been added twice.
                    // can't recreate on test forum, just add this fix logic
                    $bburl = $GLOBALS['vbulletin']->options['bburl'] . '/';

                    if (strpos($attachment_url, $bburl . $bburl) !== false)
                    {
                        $attachment_url = str_replace($bburl . $bburl, $bburl, $attachment_url);
                    }
                    if (strpos($attachment_thumbnail_url, $bburl . $bburl) !== false)
                    {
                        $attachment_thumbnail_url = str_replace($bburl . $bburl, $bburl, $attachment_thumbnail_url);
                    }
            ...
            if($html_content){
                ...
                $post_content = unhtmlspecialchars($post_content);
