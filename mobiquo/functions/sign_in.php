...
function sign_in_func($xmlrpc_params)
{
    ...
    if ($vbulletin->options['tapatalk_reg_type'] == 1 && !empty($InputParams['password'])) {
        $result = new xmlrpcval(array(
            'result'        => new xmlrpcval(false, 'boolean'),
            'result_text'   => new xmlrpcval('social sign is disabled', 'base64'),
        ), 'struct');
    ...
    if (true)
    {
        ...
        else
        {
            ...
            $return_array = array(
                ...
                'email'             => new xmlrpcval(sha1(strtolower(mobiquo_encode($mobiquo_userinfo['email']))), 'base64'),
