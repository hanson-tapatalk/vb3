...
function get_user_by_NameorEmail($name)
{
    ...
    $query = "SELECT userid, usergroupid, membergroupids, infractiongroupids, username, email, options
      FROM " . TABLE_PREFIX . "user
    ...
    if ($vbulletin->userinfo['userid'] AND $vbulletin->userinfo['permissions']['passwordexpires'])
    {
        ...
        if ($passworddaysold >= $vbulletin->userinfo['permissions']['passwordexpires'])
        {
            $response_array = array(
                ...
                'result_reason' => new xmlrpcval(3, 'int'),
            );
...
function mobiquo_get_user_icon($userid, $thum = false)
{
    ...
    if($vbulletin->options['avatarenabled']){
        fetch_avatar_from_userinfo($userinfo,$thum,false);
