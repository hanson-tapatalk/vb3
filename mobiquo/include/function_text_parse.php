	...
	if ($has_img_code AND preg_match_all('#\[attach(?:=(right|left))?\](\d+)\[/attach\]#i', $bbcode, $matches))
	{
		...
		foreach($matches[2] AS $key => $attachmentid)
		{
			...
			if (!empty($post[attachments]["$attachmentid"]))
			{
                ...
                $attachmentUrl = unhtmlspecialchars($attachmentUrl);
			...
			else
			{
                ...
                switch($extension)
                {
                    ...
                    case 'tif':
                         {
                             $attachmentUrl = unhtmlspecialchars("{$vbulletin->options['bburl']}/attachment.php?{$vbulletin->session->vars['sessionurl_js']}attachmentid=\\1&d=$attachment[dateline]");
                             $replace[] = "[IMG]". $attachmentUrl  . "[/IMG]";
                            break;
                        }
                    default:
                        {
                            $attachmentUrl = unhtmlspecialchars("{$vbulletin->options['bburl']}/attachment.php?{$vbulletin->session->vars['sessionurl']}attachmentid=\\1&d=$attachment[dateline]");
                            $replace[] = "[URL=" . $attachmentUrl . "]$vbphrase[attachment] \\1[/URL]";
                            break;
                        }
                }
			}
