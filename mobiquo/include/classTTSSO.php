...
class TTSSOBase
{
    ...
    public function signIn($params)
    {
        ...
        $this->register = !empty($password) ? true : false;
        ...
        if ($this->register) {
            ...
            if($email && $username && $password && !empty($this->userInfo))
            {
            ...
            else
            {
                ...
                $accountIsValidated = $this->verified && $this->TTEmail == $email;
                $this->createUser($email, $username, $password, $custom_register_fields, $accountIsValidated);
                $this->loginUser();
            }
        } else {
            $this->TTVerify($token, $code);
            ...
            if ($this->verified)
            {
                ...
                if ($this->userInfo && ($this->userInfo['email'] == $this->TTEmail || sha1(strtolower($this->userInfo['email'])) == $this->TTEmail) ) // user exists, do login
                {
                    $this->loginUser();
                }
                else
    ...
    public function createUser($email, $username, $password, $custom_register_fields, $accountIsValidated)
    {
        if (empty($email))
        {
            $this->status = 2;
        ...
        else
        {
            ...
            {
                if($password = $this->validatePassword($password, $accountIsValidated))
                {
                    $this->userInfo = $this->forumInterface->createUserHandle($email, $username, $password, $accountIsValidated, $custom_register_fields, $this->TTProfile, $this->errors);
                }
                else
                {
                    $this->status = 2;
    ...
    {
        $this->userInfo = $this->forumInterface->getUserById($userId);
    }
